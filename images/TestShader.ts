...
class TestShader extends egret.DisplayObjectContainer {
    ...
    private onAddToStage(e: egret.Event) {
        ...
        this.poke1.x = this.stage.stageWidth / 2 - this.poke1.width / 2;
        ...
        //this.label.alpha = 0;
        this.label.width = this.stage.stageWidth;
    ...
    public motion: egret.Motion;
    public label: egret.TextField;
    public num: number = 15;
    public speed: number = .5;
    public onMotion(e: egret.MotionEvent) {

        var nn: number = Math.floor(e.accelerationIncludingGravity.x*10)
        this.label.text = "加速度: \n:" + nn + ",\n:" + e.accelerationIncludingGravity.y + ",\nz:" + e.accelerationIncludingGravity.z;​//  关键代码结束

        if (nn < 0&&nn>-50) {
            // this.label.y--;
            this.updateFilter(this.poke1, this.updateData1, 2, true, nn);
        } else if (nn > 0 &&nn<50 ) {
            this.updateFilter(this.poke1, this.updateData1, 2, false, nn);
        }
    ...
    private updateFilter(poke: egret.Bitmap, updateData: UpdateData, speed: number = 2, LR: Boolean = true, nn: number): void {
        poke = this.poke1;
        updateData = this.updateData1;
        let filter: egret.CustomFilter = <egret.CustomFilter>poke.filters[0];
        filter.uniforms.angle = nn*.015;
        // let tw: egret.Tween = egret.Tween.get(filter.uniforms, { "loop": false });
        // tw.to({ angle:nn*.3}, 500);
        // if (LR) {
        //     if (filter.uniforms.angle > -.5) {
        //         filter.uniforms.angle -= speed * Math.PI / 180;
        //     }
        //     //filter.uniforms.angle

        //     //this.label.text+="ss --  "+filter.uniforms.angle
        // } else {
        //     if (filter.uniforms.angle < .5) {
        //         filter.uniforms.angle += speed * Math.PI / 180;
        //     }
